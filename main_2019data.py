...
def make_folds(train_csv, config):
    ...
    df_folds = df_folds[df_folds['source']==2020].reset_index(drop=True)
...
def train_on_fold(df_folds, df_2019, config, device, fold):
...
def train_loop(df_folds: pd.DataFrame, df_2019, config, device, fold_num: int = None, train_one_fold=False):
    ...
    if train_one_fold:
        ...
        for fold in (number+1 for number in range(config.num_folds)):
            _oof_df = train_on_fold(df_folds=df_folds, df_2019=df2019, config=config, device=device, fold=fold)
            oof_df = pd.concat([oof_df, _oof_df])
            curr_fold_best_score = get_acc_score(config, _oof_df)
...
if __name__ == '__main__':
    ...
    df2019 = train_csv[train_csv['source']==2019].reset_index(drop=True)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    train_five_folds = train_loop(df_folds=df_folds, df_2019=df2019, config=config, device=device,
